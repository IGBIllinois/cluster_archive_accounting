...
class data_functions {
	...
	public static function get_data_bill($db,$month,$year) {
		...
		$sql = "SELECT users.username as Username, directories.directory as Directory, ROUND(archive_usage.directory_size / 1048576,3) as `Usage`, archive_usage.cost as Cost, (select sum(transactions.amount) from transactions where transactions.directory_id=directories.id and ((month(transaction_time)<=:month and year(transaction_time)=:year) or year(transaction_time)<:year)) as Balance, cfops.cfop as CFOP FROM archive_usage LEFT JOIN directories on archive_usage.directory_id=directories.id LEFT JOIN users ON directories.user_id=users.id left join cfops on cfops.directory_id=directories.id WHERE YEAR(archive_usage.usage_time)=:year AND MONTH(archive_usage.usage_time)=:month and directories.do_not_bill=0 and cfops.active=1 group by directories.id ORDER BY Directory ASC";
